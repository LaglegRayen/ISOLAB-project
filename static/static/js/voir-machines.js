...
document.addEventListener('DOMContentLoaded', function() {
    console.log('DOM Content Loaded - voir-machines.js script starting');
...
function checkMachineParameter() {
    ...
    if (machineId) {
        ...
        // If there's an add-machine action, open the add machine modal
        setTimeout(() => {
            addNewMachine();
        }, 500);
    }
...
function addNewMachine() {
    console.log('addNewMachine called');
    openAddMachineModal();
}

// Modal functions
function openAddMachineModal() {
    console.log('openAddMachineModal called');
    ...
    if (modal) {
        ...
        // Load and display all clients immediately when modal opens
        console.log('Loading all clients for selection...');
        loadAllClientsForSelection();
    } else {
        console.error('Modal element not found! Check if addMachineModal exists in HTML');
    }
}
...
function initializeClientSearch() {
    console.log('initializeClientSearch called');
    const clientSearch = document.getElementById('clientSearch');
    console.log('Client search input found:', !!clientSearch);
    
    if (clientSearch) {
        console.log('Adding input event listener to client search for filtering');
        clientSearch.addEventListener('input', function() {
            const query = this.value.trim();
            console.log('Client search input event triggered, query:', query, 'length:', query.length);
            if (query.length >= 0) { // Changed from >= 2 to >= 0 to allow filtering from first character
                console.log('Filtering clients with query:', query);
                filterDisplayedClients(query);
            }
        });
    } else {
...
// Load all clients and display them in the modal
async function loadAllClientsForSelection() {
    try {
        console.log('loadAllClientsForSelection called');
        console.log('Fetching from /clients/all...');
        ...
        window.allClientsForSelection = clients;
        
        // Display all clients immediately
        ...
        if (resultsContainer) {
            resultsContainer.innerHTML = '<div class="no-results">Erreur de connexion - Vérifiez que le serveur est démarré</div>';
            resultsContainer.style.display = 'block';
        }
    }
}

// Filter the already displayed clients based on search input
function filterDisplayedClients(query) {
    const allClients = window.allClientsForSelection || [];
    
    if (!query) {
        // If no query, show all clients
        displayClientSearchResults(allClients);
        return;
    }
    ...
    const filteredClients = allClients.filter(client => {
        ...
        const societyMatch = client.clientSociety && client.clientSociety.toLowerCase().includes(searchLower);
        const nameMatch = client.clientName && client.clientName.toLowerCase().includes(searchLower);
        const phoneMatch = client.clientPhone && client.clientPhone.includes(query);
        console.log('Match results - society:', societyMatch, 'name:', nameMatch, 'phone:', phoneMatch);
        return societyMatch || nameMatch || phoneMatch;
    });
...
async function searchClients(query) {
    try {
        console.log('searchClients called with query:', query);
        console.log('Fetching from /clients/all...');
        
        // First test if we can reach the server at all
        console.log('Testing server connectivity...');
        const testResponse = await fetch(`${API_BASE_URL}/users/current`, { 
            credentials: 'include',
            headers: {
                'Content-Type': 'application/json'
            }
        });
        console.log('Test response (users/current):', testResponse.status, testResponse.statusText);
        ...
        console.log('Extracted clients array:', clients);
        console.log('Number of clients:', clients.length);
        console.log('Type of clients:', typeof clients, Array.isArray(clients));
...
function displayClientSearchResults(clients) {
    console.log('displayClientSearchResults called with:', clients);
    const resultsContainer = document.getElementById('clientSearchResults');
    console.log('Results container found:', !!resultsContainer);
    
    if (clients.length === 0) {
        console.log('No clients found, showing no results message');
        ...
        resultsContainer.innerHTML = clients.map(client => {
            ...
            // Escape quotes and handle null values for safety
            const clientId = client.id || '';
            const clientSociety = (client.clientSociety || 'N/A').replace(/'/g, '&apos;').replace(/"/g, '&quot;');
            const clientName = (client.clientName || 'N/A').replace(/'/g, '&apos;').replace(/"/g, '&quot;');
            const clientPhone = client.clientPhone || 'N/A';
            const clientAddress = (client.clientAddress || '').replace(/'/g, '&apos;').replace(/"/g, '&quot;');
            const clientLocation = client.clientLocation || 'N/A';
            
            return `
            <div class="search-result-item" onclick="selectClient('${clientId}', '${clientSociety}', '${clientName}', '${clientPhone}', '${clientAddress}')">
                <div class="client-info">
                    <strong>${client.clientSociety || 'N/A'}</strong>
                </div>
                <div class="client-details">
                    <span><i class="fas fa-user"></i> ${client.clientName || 'N/A'}</span>
                    <span><i class="fas fa-phone"></i> ${client.clientPhone || 'N/A'}</span>
                    <span><i class="fas fa-map-marker"></i> ${clientLocation}</span>
                </div>
            </div>
        `;}).join('');
    }
    ...
    resultsContainer.style.display = 'block';
    console.log('Results container final HTML:', resultsContainer.innerHTML);
}

function selectClient(id, clientSociety, clientName, clientPhone, clientAddress) {
    selectedClient = { id, clientSociety, clientName, clientPhone, clientAddress };
    
    document.getElementById('selectedClientInfo').innerHTML = `
        <div class="selected-client-info">
            <div class="client-header">
                <strong>${clientSociety}</strong>
            ...
            <div class="client-details">
                ...
                <p><strong>Adresse:</strong> ${clientAddress || 'N/A'}</p>
            </div>
        </div>
    `;
    
    document.getElementById('clientSearchResults').style.display = 'none';
    document.getElementById('clientSearch').value = clientSociety;
}
...
async function submitMachineForm() {
    ...
    const serialNumber = document.getElementById('formSerialNumber').value.trim();
    const ficheNumber = document.getElementById('formFicheNumber').value.trim();
    const machineType = document.getElementById('formMachineType').value.trim();
    const prixHT = document.getElementById('formPrixHT').value.trim();
    const prixTTC = document.getElementById('formPrixTTC').value.trim();
    ...
    const confirmation = document.getElementById('formConfirmation').value;
    const facturation = document.getElementById('formFacturation').value;
    const remarques = document.getElementById('formRemarques').value.trim();
    
    // Debug: Log the form values to console
    console.log('Form values:', {
        serialNumber: `"${serialNumber}"`,
        ficheNumber: `"${ficheNumber}"`,
        machineType: `"${machineType}"`,
        serialNumberLength: serialNumber.length,
        ficheNumberLength: ficheNumber.length,
        machineTypeLength: machineType.length
    });
    
    // Basic validation
    if (!serialNumber || !ficheNumber || !machineType) {
        console.error('Validation failed:', {
            serialNumber: !!serialNumber,
            ficheNumber: !!ficheNumber,
            machineType: !!machineType
        });
        alert('Veuillez remplir les champs obligatoires (numéro de série, numéro de fiche et type de machine)');
        return;
    }
    
    // Create form data that matches the backend structure exactly
    const formData = {
        serialNumber: serialNumber,
        ficheNumber: ficheNumber,
        machineType: machineType,
        ...
        clientSociety: selectedClient.clientSociety,
        prixHT: prixHT ? parseFloat(prixHT) : 0,
        prixTTC: prixTTC ? parseFloat(prixTTC) : 0,
        paymentType: paymentType || 'Crédit',
        paymentStatus: paymentStatus || 'En cours',
        confirmation: confirmation || 'En attente',
        facturation: facturation || 'Non facturée',
        remarques: remarques || ''
    };
    ...
    try {
        ...
        if (response.ok) {
            alert(`Machine ajoutée avec succès!\nAssignée à: ${result.assigned_to}\nÉtape actuelle: ${result.current_stage}`);
        ...
        alert('Erreur lors de l\'ajout de la machine. Vérifiez que le serveur est démarré.');
    }
}
...
async function updateWorkflowStage(stageName, newStatus) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/workflows/${currentMachineId}/stage/${stageName}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
            },
